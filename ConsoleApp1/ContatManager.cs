using System;
using System.Collections.Generic;
using System.Text;
using System.IO;
using System.Linq;

namespace ConsoleApp1
{
    class ContatManager
    {

       private List<ContactEntry> _entries;  //Это все поле -  Класс List , Тип ContactEntry ,имя _entries
       private string _filePath;

        public ContatManager()
        {
           _entries = new List<ContactEntry>();
           _filePath = "";
        }

        public ContatManager(string filePath)
        {
            _entries = new List<ContactEntry>();
            ReadFromFile(filePath);
            _filePath = filePath;
        }
        public bool ReadFromFile (string filePath)
        {
            _filePath = filePath;
            if (!File.Exists(filePath))
            {
                return false;
            }


            StreamReader st = File.OpenText(_filePath);

            while (!st.EndOfStream)
            {
                this.addEntryFromFileLine(st.ReadLine());
            }

            return true;
        }
        public void AddEntry(ContactEntry contact)
        {
            _entries.Add(contact);
        }
        public void AddEntry(ContactEntry contact, bool forceCommit)
        {
            AddEntry(contact);
            if (forceCommit)
            {
                Commit();
            }
        }

        public void Commit()
        {
            StreamWriter sw = File.CreateText(_filePath);

            foreach (ContactEntry ce in _entries)
            {
                sw.WriteLine(string.Format("{0}|{1}|{2}|{3}",
                    ce.LastName,
                    ce.FirstName,
                    ce.PhoneNumber,
                    ce.Email));
            }

            sw.Close();
        }

        public ContactEntry SerchForContact (string LastName, string FirstName)
        {
            foreach (ContactEntry ce in _entries)
                if (ce.LastName == LastName && ce.FirstName == FirstName)
                    return ce;
            return null;
        }
        public void RemoveEntry(ContactEntry contact)
        {
            _entries.Remove(contact);
        }

        private void addEntryFromFileLine (string line)
        {
          string[] fields = line.Split('|');

            ContactEntry ce = new ContactEntry();
            ce.LastName = fields[0];
            ce.FirstName = fields[1];
            ce.PhoneNumber = fields[2];
            ce.Email = fields[3];

            _entries.Add(ce);
        }



    }

}
